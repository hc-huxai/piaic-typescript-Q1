var $5ryz8$process = require("process");
var $5ryz8$buffer = require("buffer");
var $5ryz8$fs = require("fs");
var $5ryz8$nodetimerspromises = require("node:timers/promises");
var $5ryz8$nodeprocess = require("node:process");
var $5ryz8$nodereadline = require("node:readline");
var $5ryz8$nodetty = require("node:tty");
...
var $51a101c4cf49dfb6$require$Buffer = $5ryz8$buffer.Buffer;
"use strict";
...
function $51a101c4cf49dfb6$var$create(config) {
   ...
   */ function prompt(ask, value, opts) {
        ...
        var fd = $5ryz8$process.platform === "win32" ? $5ryz8$process.stdin.fd : $5ryz8$fs.openSync("/dev/tty", "rs");
        var wasRaw = $5ryz8$process.stdin.isRaw;
        if (!wasRaw) $5ryz8$process.stdin.setRawMode && $5ryz8$process.stdin.setRawMode(true);
        var buf = $51a101c4cf49dfb6$require$Buffer.alloc(3);
        var str = "", character, read;
        savedstr = "";
        if (ask) $5ryz8$process.stdout.write(ask);
        var cycle = 0;
        var prevComplete;
        while(true){
            read = $5ryz8$fs.readSync(fd, buf, 0, 3);
            if (read > 1) {
                switch(buf.toString()){
                    case "\x1b[A":
                        if (masked) break;
                        ...
                        str = history.prev();
                        insert = str.length;
                        $5ryz8$process.stdout.write("\x1b[2K\x1b[0G" + ask + str);
                        break;
                    case "\x1b[B":
                        if (masked) break;
                        if (!history) break;
                        ...
                        $5ryz8$process.stdout.write("\x1b[2K\x1b[0G" + ask + str + "\x1b[" + (insert + ask.length + 1) + "G");
                        break;
                    case "\x1b[D":
                        if (masked) break;
                        var before = insert;
                        insert = --insert < 0 ? 0 : insert;
                        if (before - insert) $5ryz8$process.stdout.write("\x1b[1D");
                        break;
                    case "\x1b[C":
                        if (masked) break;
                        insert = ++insert > str.length ? str.length : insert;
                        $5ryz8$process.stdout.write("\x1b[" + (insert + ask.length + 1) + "G");
                        break;
                    default:
                        if (buf.toString()) {
            ...
            if (character == 3) {
                $5ryz8$process.stdout.write("^C\n");
                $5ryz8$fs.closeSync(fd);
                if (sigint) $5ryz8$process.exit(130);
                $5ryz8$process.stdin.setRawMode && $5ryz8$process.stdin.setRawMode(wasRaw);
                return null;
            }
            // catch a ^D and exit
            if (character == 4) {
                if (str.length == 0 && eot) {
                    $5ryz8$process.stdout.write("exit\n");
                    $5ryz8$process.exit(0);
                }
            }
            ...
            if (character == 9) {
                ...
                if (res.length == 0) {
                    $5ryz8$process.stdout.write("	");
                    continue;
                }
                var item = res[cycle++] || res[cycle = 0, cycle++];
                if (item) {
                    $5ryz8$process.stdout.write("\r\x1b[K" + ask + item);
                    str = item;
                    insert = item.length;
                }
            }
            if (character == 127 || $5ryz8$process.platform == "win32" && character == 8) {
                if (!insert) continue;
                str = str.slice(0, insert - 1) + str.slice(insert);
                insert--;
                $5ryz8$process.stdout.write("\x1b[2D");
            } else {
    ...
    function promptPrint(masked, ask, echo, str, insert) {
        if (masked) $5ryz8$process.stdout.write("\x1b[2K\x1b[0G" + ask + Array(str.length + 1).join(echo));
        else {
            $5ryz8$process.stdout.write("\x1b[s");
            if (insert == str.length) $5ryz8$process.stdout.write("\x1b[2K\x1b[0G" + ask + str);
            else if (ask) $5ryz8$process.stdout.write("\x1b[2K\x1b[0G" + ask + str);
            else $5ryz8$process.stdout.write("\x1b[2K\x1b[0G" + str + "\x1b[" + (str.length - insert) + "D");
            // Reposition the cursor to the right of the insertion point
            var askLength = $1YCa9(ask).length;
            $5ryz8$process.stdout.write(`\u001b[${askLength + 1 + (echo == "" ? 0 : insert)}G`);
        }
    }
}
...
$parcel$export(module.exports, "default", () => $811b16d3d31cae02$export$2e2bcd8739ae039);
/**
...
class $19a71fb7f114ebdf$export$83716a4aa1642908 {
    constructor({ render: u, input: F = (0, $5ryz8$nodeprocess.stdin), output: e = (0, $5ryz8$nodeprocess.stdout), ...s }, C = !0){
...
`), $19a71fb7f114ebdf$var$g(this.input, !1), this.rl.close(), this.emit(`${this.state}`, this.value), this.unsubscribe();
    ...
    restoreCursor() {
        const u = $19a71fb7f114ebdf$var$P(this._prevFrame, $5ryz8$process.stdout.columns, {
            hard: !0
        }).split(`
`).length - 1;
    ...
    render() {
        const u = $19a71fb7f114ebdf$var$P(this._render(this) ?? "", $5ryz8$process.stdout.columns, {
            hard: !0
        });
...
function $19a71fb7f114ebdf$export$837bd02682cd3db9({ input: t1 = (0, $5ryz8$nodeprocess.stdin), output: u = (0, $5ryz8$nodeprocess.stdout), overwrite: F = !0, hideCursor: e = !0 } = {}) {
    const s = $5ryz8$nodereadline.createInterface({
        input: t1,
        output: u,
        prompt: "",
        tabSize: 1
    });
    $5ryz8$nodereadline.emitKeypressEvents(t1, s), t1.isTTY && t1.setRawMode(!0);
    const C = (D, { name: i })=>{
        if (String(D) === "\x03" && $5ryz8$process.exit(0), !F) return;
        let E = i === "return" ? 0 : -1, a = i === "return" ? -1 : 0;
        $5ryz8$nodereadline.moveCursor(u, E, a, ()=>{
            $5ryz8$nodereadline.clearLine(u, 1, ()=>{
                t1.once("keypress", C);
            });
        });
    };
    return e && $5ryz8$process.stdout.write((0, $2266340f27e5c1f6$exports.cursor).hide), t1.once("keypress", C), ()=>{
        t1.off("keypress", C), e && $5ryz8$process.stdout.write((0, $2266340f27e5c1f6$exports.cursor).show), t1.isTTY && t1.setRawMode(!1), s.terminal = !1, s.close();
    };
}
...
function $77cdac647ae9de8c$var$q() {
    return (0, ($parcel$interopDefault($5ryz8$nodeprocess))).platform !== "win32" ? (0, ($parcel$interopDefault($5ryz8$nodeprocess))).env.TERM !== "linux" : Boolean((0, ($parcel$interopDefault($5ryz8$nodeprocess))).env.CI) || Boolean((0, ($parcel$interopDefault($5ryz8$nodeprocess))).env.WT_SESSION) || Boolean((0, ($parcel$interopDefault($5ryz8$nodeprocess))).env.TERMINUS_SUBLIME) || (0, ($parcel$interopDefault($5ryz8$nodeprocess))).env.ConEmuTask === "{cmd::Cmder}" || (0, ($parcel$interopDefault($5ryz8$nodeprocess))).env.TERM_PROGRAM === "Terminus-Sublime" || (0, ($parcel$interopDefault($5ryz8$nodeprocess))).env.TERM_PROGRAM === "vscode" || (0, ($parcel$interopDefault($5ryz8$nodeprocess))).env.TERM === "xterm-256color" || (0, ($parcel$interopDefault($5ryz8$nodeprocess))).env.TERM === "alacritty" || (0, ($parcel$interopDefault($5ryz8$nodeprocess))).env.TERMINAL_EMULATOR === "JetBrains-JediTerm";
}
...
`);
    $5ryz8$process.stdout.write(`${(0, (/*@__PURE__*/$parcel$interopDefault($dfe518de425e986f$exports))).gray($77cdac647ae9de8c$var$a)}
...
`);
    ...
    $5ryz8$process.stdout.write(`${(0, (/*@__PURE__*/$parcel$interopDefault($dfe518de425e986f$exports))).gray($77cdac647ae9de8c$var$d)}  ${(0, (/*@__PURE__*/$parcel$interopDefault($dfe518de425e986f$exports))).red(r)}

`);
}, $77cdac647ae9de8c$export$affaf52642033041 = (r = "")=>{
    $5ryz8$process.stdout.write(`${(0, (/*@__PURE__*/$parcel$interopDefault($dfe518de425e986f$exports))).gray($77cdac647ae9de8c$var$K)}  ${r}
`);
...
`);
        ...
        $5ryz8$process.stdout.write(`${i.join(`
`)}
`);
    },
    info: (r)=>{
    ...
    let i, t, s = !1, c = "";
    const l = (v = "")=>{
        s = !0, i = (0, $19a71fb7f114ebdf$export$837bd02682cd3db9)(), c = v.replace(/\.+$/, ""), $5ryz8$process.stdout.write(`${(0, (/*@__PURE__*/$parcel$interopDefault($dfe518de425e986f$exports))).gray($77cdac647ae9de8c$var$a)}
`);
        let g = 0, p = 0;
        t = setInterval(()=>{
            const O = (0, (/*@__PURE__*/$parcel$interopDefault($dfe518de425e986f$exports))).magenta(r[g]), P = ".".repeat(Math.floor(p)).slice(0, 3);
            $5ryz8$process.stdout.write((0, $2266340f27e5c1f6$exports.cursor).move(-999, 0)), $5ryz8$process.stdout.write((0, $2266340f27e5c1f6$exports.erase).down(1)), $5ryz8$process.stdout.write(`${O}  ${c}${P}`), g = g + 1 < r.length ? g + 1 : 0, p = p < r.length ? p + .125 : 0;
        }, n);
    }, u = (v = "", g = 0)=>{
        ...
        $5ryz8$process.stdout.write((0, $2266340f27e5c1f6$exports.cursor).move(-999, 0)), $5ryz8$process.stdout.write((0, $2266340f27e5c1f6$exports.erase).down(1)), $5ryz8$process.stdout.write(`${p}  ${c}
`), i();
    }, m = (v = "")=>{
        c = v ?? c;
    }, $ = (v)=>{
    ...
    return $5ryz8$process.on("uncaughtExceptionMonitor", ()=>$(2)), $5ryz8$process.on("unhandledRejection", ()=>$(2)), $5ryz8$process.on("SIGINT", ()=>$(1)), $5ryz8$process.on("SIGTERM", ()=>$(1)), $5ryz8$process.on("exit", $), {
        start: l,
        stop: u,
        message: m
    };
...
const $07b3886a9cba6ba1$var$Spinner = async (timeout)=>{
    ...
    s.start();
    await (0, $5ryz8$nodetimerspromises.setTimeout)(timeout == undefined ? 2000 : timeout);
    s.stop();
};
...
const $811b16d3d31cae02$var$Exercise14 = ()=>{
    let guests = [
        "Abu Hurairah",
        "Hamza",
        "Osman"
...
$811b16d3d31cae02$var$Exercise14();
var $811b16d3d31cae02$export$2e2bcd8739ae039 = $811b16d3d31cae02$var$Exercise14;
