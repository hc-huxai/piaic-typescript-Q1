var $f07dH$process = require("process");
var $f07dH$buffer = require("buffer");
var $f07dH$fs = require("fs");
var $f07dH$nodetimerspromises = require("node:timers/promises");
var $f07dH$nodeprocess = require("node:process");
var $f07dH$nodereadline = require("node:readline");
var $f07dH$nodetty = require("node:tty");
...
var $51a101c4cf49dfb6$require$Buffer = $f07dH$buffer.Buffer;
"use strict";
...
function $51a101c4cf49dfb6$var$create(config) {
   ...
   */ function prompt(ask, value, opts) {
        ...
        var fd = $f07dH$process.platform === "win32" ? $f07dH$process.stdin.fd : $f07dH$fs.openSync("/dev/tty", "rs");
        var wasRaw = $f07dH$process.stdin.isRaw;
        if (!wasRaw) $f07dH$process.stdin.setRawMode && $f07dH$process.stdin.setRawMode(true);
        var buf = $51a101c4cf49dfb6$require$Buffer.alloc(3);
        var str = "", character, read;
        savedstr = "";
        if (ask) $f07dH$process.stdout.write(ask);
        var cycle = 0;
        var prevComplete;
        while(true){
            read = $f07dH$fs.readSync(fd, buf, 0, 3);
            if (read > 1) {
                switch(buf.toString()){
                    case "\x1b[A":
                        if (masked) break;
                        ...
                        str = history.prev();
                        insert = str.length;
                        $f07dH$process.stdout.write("\x1b[2K\x1b[0G" + ask + str);
                        break;
                    case "\x1b[B":
                        if (masked) break;
                        if (!history) break;
                        ...
                        $f07dH$process.stdout.write("\x1b[2K\x1b[0G" + ask + str + "\x1b[" + (insert + ask.length + 1) + "G");
                        break;
                    case "\x1b[D":
                        if (masked) break;
                        var before = insert;
                        insert = --insert < 0 ? 0 : insert;
                        if (before - insert) $f07dH$process.stdout.write("\x1b[1D");
                        break;
                    case "\x1b[C":
                        if (masked) break;
                        insert = ++insert > str.length ? str.length : insert;
                        $f07dH$process.stdout.write("\x1b[" + (insert + ask.length + 1) + "G");
                        break;
                    default:
                        if (buf.toString()) {
            ...
            if (character == 3) {
                $f07dH$process.stdout.write("^C\n");
                $f07dH$fs.closeSync(fd);
                if (sigint) $f07dH$process.exit(130);
                $f07dH$process.stdin.setRawMode && $f07dH$process.stdin.setRawMode(wasRaw);
                return null;
            }
            // catch a ^D and exit
            if (character == 4) {
                if (str.length == 0 && eot) {
                    $f07dH$process.stdout.write("exit\n");
                    $f07dH$process.exit(0);
                }
            }
            ...
            if (character == 9) {
                ...
                if (res.length == 0) {
                    $f07dH$process.stdout.write("	");
                    continue;
                }
                var item = res[cycle++] || res[cycle = 0, cycle++];
                if (item) {
                    $f07dH$process.stdout.write("\r\x1b[K" + ask + item);
                    str = item;
                    insert = item.length;
                }
            }
            if (character == 127 || $f07dH$process.platform == "win32" && character == 8) {
                if (!insert) continue;
                str = str.slice(0, insert - 1) + str.slice(insert);
                insert--;
                $f07dH$process.stdout.write("\x1b[2D");
            } else {
    ...
    function promptPrint(masked, ask, echo, str, insert) {
        if (masked) $f07dH$process.stdout.write("\x1b[2K\x1b[0G" + ask + Array(str.length + 1).join(echo));
        else {
            $f07dH$process.stdout.write("\x1b[s");
            if (insert == str.length) $f07dH$process.stdout.write("\x1b[2K\x1b[0G" + ask + str);
            else if (ask) $f07dH$process.stdout.write("\x1b[2K\x1b[0G" + ask + str);
            else $f07dH$process.stdout.write("\x1b[2K\x1b[0G" + str + "\x1b[" + (str.length - insert) + "D");
            // Reposition the cursor to the right of the insertion point
            var askLength = $1YCa9(ask).length;
            $f07dH$process.stdout.write(`\u001b[${askLength + 1 + (echo == "" ? 0 : insert)}G`);
        }
    }
}
...
$parcel$export(module.exports, "default", () => $cb3b141016bbc8ae$export$2e2bcd8739ae039);
/**
...
class $19a71fb7f114ebdf$export$83716a4aa1642908 {
    constructor({ render: u, input: F = (0, $f07dH$nodeprocess.stdin), output: e = (0, $f07dH$nodeprocess.stdout), ...s }, C = !0){
...
`), $19a71fb7f114ebdf$var$g(this.input, !1), this.rl.close(), this.emit(`${this.state}`, this.value), this.unsubscribe();
    ...
    restoreCursor() {
        const u = $19a71fb7f114ebdf$var$P(this._prevFrame, $f07dH$process.stdout.columns, {
            hard: !0
        }).split(`
`).length - 1;
    ...
    render() {
        const u = $19a71fb7f114ebdf$var$P(this._render(this) ?? "", $f07dH$process.stdout.columns, {
            hard: !0
        });
...
function $19a71fb7f114ebdf$export$837bd02682cd3db9({ input: t1 = (0, $f07dH$nodeprocess.stdin), output: u = (0, $f07dH$nodeprocess.stdout), overwrite: F = !0, hideCursor: e = !0 } = {}) {
    const s = $f07dH$nodereadline.createInterface({
        input: t1,
        output: u,
        prompt: "",
        tabSize: 1
    });
    $f07dH$nodereadline.emitKeypressEvents(t1, s), t1.isTTY && t1.setRawMode(!0);
    const C = (D, { name: i })=>{
        if (String(D) === "\x03" && $f07dH$process.exit(0), !F) return;
        let E = i === "return" ? 0 : -1, a = i === "return" ? -1 : 0;
        $f07dH$nodereadline.moveCursor(u, E, a, ()=>{
            $f07dH$nodereadline.clearLine(u, 1, ()=>{
                t1.once("keypress", C);
            });
        });
    };
    return e && $f07dH$process.stdout.write((0, $2266340f27e5c1f6$exports.cursor).hide), t1.once("keypress", C), ()=>{
        t1.off("keypress", C), e && $f07dH$process.stdout.write((0, $2266340f27e5c1f6$exports.cursor).show), t1.isTTY && t1.setRawMode(!1), s.terminal = !1, s.close();
    };
}
...
function $77cdac647ae9de8c$var$q() {
    return (0, ($parcel$interopDefault($f07dH$nodeprocess))).platform !== "win32" ? (0, ($parcel$interopDefault($f07dH$nodeprocess))).env.TERM !== "linux" : Boolean((0, ($parcel$interopDefault($f07dH$nodeprocess))).env.CI) || Boolean((0, ($parcel$interopDefault($f07dH$nodeprocess))).env.WT_SESSION) || Boolean((0, ($parcel$interopDefault($f07dH$nodeprocess))).env.TERMINUS_SUBLIME) || (0, ($parcel$interopDefault($f07dH$nodeprocess))).env.ConEmuTask === "{cmd::Cmder}" || (0, ($parcel$interopDefault($f07dH$nodeprocess))).env.TERM_PROGRAM === "Terminus-Sublime" || (0, ($parcel$interopDefault($f07dH$nodeprocess))).env.TERM_PROGRAM === "vscode" || (0, ($parcel$interopDefault($f07dH$nodeprocess))).env.TERM === "xterm-256color" || (0, ($parcel$interopDefault($f07dH$nodeprocess))).env.TERM === "alacritty" || (0, ($parcel$interopDefault($f07dH$nodeprocess))).env.TERMINAL_EMULATOR === "JetBrains-JediTerm";
}
...
`);
    $f07dH$process.stdout.write(`${(0, (/*@__PURE__*/$parcel$interopDefault($dfe518de425e986f$exports))).gray($77cdac647ae9de8c$var$a)}
...
`);
    ...
    $f07dH$process.stdout.write(`${(0, (/*@__PURE__*/$parcel$interopDefault($dfe518de425e986f$exports))).gray($77cdac647ae9de8c$var$d)}  ${(0, (/*@__PURE__*/$parcel$interopDefault($dfe518de425e986f$exports))).red(r)}

`);
}, $77cdac647ae9de8c$export$affaf52642033041 = (r = "")=>{
    $f07dH$process.stdout.write(`${(0, (/*@__PURE__*/$parcel$interopDefault($dfe518de425e986f$exports))).gray($77cdac647ae9de8c$var$K)}  ${r}
`);
...
`);
        ...
        $f07dH$process.stdout.write(`${i.join(`
`)}
`);
    },
    info: (r)=>{
    ...
    let i, t, s = !1, c = "";
    const l = (v = "")=>{
        s = !0, i = (0, $19a71fb7f114ebdf$export$837bd02682cd3db9)(), c = v.replace(/\.+$/, ""), $f07dH$process.stdout.write(`${(0, (/*@__PURE__*/$parcel$interopDefault($dfe518de425e986f$exports))).gray($77cdac647ae9de8c$var$a)}
`);
        let g = 0, p = 0;
        t = setInterval(()=>{
            const O = (0, (/*@__PURE__*/$parcel$interopDefault($dfe518de425e986f$exports))).magenta(r[g]), P = ".".repeat(Math.floor(p)).slice(0, 3);
            $f07dH$process.stdout.write((0, $2266340f27e5c1f6$exports.cursor).move(-999, 0)), $f07dH$process.stdout.write((0, $2266340f27e5c1f6$exports.erase).down(1)), $f07dH$process.stdout.write(`${O}  ${c}${P}`), g = g + 1 < r.length ? g + 1 : 0, p = p < r.length ? p + .125 : 0;
        }, n);
    }, u = (v = "", g = 0)=>{
        ...
        $f07dH$process.stdout.write((0, $2266340f27e5c1f6$exports.cursor).move(-999, 0)), $f07dH$process.stdout.write((0, $2266340f27e5c1f6$exports.erase).down(1)), $f07dH$process.stdout.write(`${p}  ${c}
`), i();
    }, m = (v = "")=>{
        c = v ?? c;
    }, $ = (v)=>{
    ...
    return $f07dH$process.on("uncaughtExceptionMonitor", ()=>$(2)), $f07dH$process.on("unhandledRejection", ()=>$(2)), $f07dH$process.on("SIGINT", ()=>$(1)), $f07dH$process.on("SIGTERM", ()=>$(1)), $f07dH$process.on("exit", $), {
        start: l,
        stop: u,
        message: m
    };
...
const $07b3886a9cba6ba1$var$Spinner = async (timeout)=>{
    ...
    s.start();
    await (0, $f07dH$nodetimerspromises.setTimeout)(timeout == undefined ? 2000 : timeout);
    s.stop();
};
...
const $cb3b141016bbc8ae$var$Exercise31 = ()=>{
    let users = [
        true,
        false
    ...
    if (users.length) users.forEach((user)=>(0, $7a54dd91ebf681ea$export$2e2bcd8739ae039)(()=>greeting(user)));
    else (0, $7a54dd91ebf681ea$export$2e2bcd8739ae039)(()=>console.log(`We need to find some users!`));
};
// TODO: Uncomment the statement below to execute the exercise function
$cb3b141016bbc8ae$var$Exercise31();
var $cb3b141016bbc8ae$export$2e2bcd8739ae039 = $cb3b141016bbc8ae$var$Exercise31;
